...
**Status: All 5 core phases complete** (33/42 items fully implemented; 9 items descoped or partial — see inline notes below). Local-LLM/Ollama and ngrok remote exposure were never implemented in this build; the architecture uses a direct Q-table policy and a local stdio MCP server instead. (228 unit + integration tests, 99% code coverage)
...
- [ ] Set up local Ollama instance with a small model pulled and confirmed
      ...
      *(Descoped: no LLM used in this implementation — Q-agent selects actions directly via epsilon-greedy)*
...
- [ ] Implement separate Thief MCP server.
      *(Descoped: single unified server with a role parameter used instead)*
...
- [ ] Expose MCP server via ngrok tunnel (or equivalent) and confirm remote 
      reachability.
      *(Descoped: local stdio MCP server only, no remote exposure)*
...
- [ ] Wire the local LLM (Ollama) into the agent decision loop using the
      ...
      *(Descoped: epsilon-greedy policy selects actions directly from Q-table)*
- [ ] Pin LLM sampling (temperature 0 / fixed seed) for reproducibility.
      *(Descoped: no LLM in decision loop)*
...
- [~] Implement per-game logging (outcome, reward, moves-to-capture) to 
      `results/tournament_report.json`.
      *(Partial: per-game logging complete, per-turn logging not implemented)*
...
- [ ] Re-run the same experiment against ngrok-tunneled (cloud-exposed) MCP
      ...
      *(Descoped: local stdio MCP server only)*
- [~] Verify reproducibility: infrastructure supports seeded runs (Board and 
      ...
      does not currently pin a seed.
      *(Partial: Board/QLearningAgent support seeded random.Random, but run_tournament.py does not currently pin a seed)*
...
- [ ] Measure and record token usage per turn/per run against the FinOps
      ...
      *(Descoped: no LLM, no tokens to measure)*
