...
This is what makes the process a Dec-POMDP rather than a Dec-MDP. Each agent's observation always includes its own exact position, the full static barrier layout (barriers are fixed per episode, so treating them as public knowledge keeps the interesting uncertainty confined to the opponent's position), and the current move count. The opponent's position is revealed **only if within a fixed Manhattan-distance visibility radius**, `VISIBILITY_RADIUS = 2` (`src/agents/state_encoding.py`) — otherwise the observation reports the opponent as unseen (`None`). The visibility rule is symmetric (same radius for both roles), so the game's only built-in asymmetry is in the reward function, not in sensing.
